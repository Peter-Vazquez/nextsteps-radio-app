(function () {
  ...
  function render(data) {
    ...
    const kpis = [
      ...
      ['Next SEAP deadline', 'Sep 7', 'Business Strategy due September 7, 2026']
    ];
    ...
    document.getElementById('training-progress-copy').innerHTML = `<strong>${data.training.loggedHours} of ${data.training.totalRequiredHours} total hours documented</strong><p>${data.training.eligibleHours || 0} hour(s) verified eligible; ${data.training.pendingHours || 0} hour(s) pending verification/acceptance; ${data.training.remainingHours ?? Math.max(0, data.training.totalRequiredHours - data.training.loggedHours)} hour(s) still required. The first 10-hour ES161.4 package was submitted August 31 at 9:26 PM; DOL acceptance remains pending.</p>`;
    ...
    document.getElementById('capacity-summary').innerHTML = `<p><strong>Accountability model:</strong> Outcome-based</p><p>${data.workload.capacityStatus}</p><p><strong>Capacity signals:</strong> client load, deliverable quality, pipeline movement, deadlines, compliance, cash, and risk.</p>`;
