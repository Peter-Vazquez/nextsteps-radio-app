...
const SPREADSHEET_ID = '19nKETpDwsD1kw267zcUH6_4bmSTMLsQY79mdfsmYZyU';
...
export async function readActionRows() {
  const encoded = encodeURIComponent(CURRENT_RANGE);
  const result = await googleFetch(`/values/${encoded}?majorDimension=ROWS`);
...
export async function saveActionRow(values) {
  ...
  return googleFetch(`/values/${target}?valueInputOption=USER_ENTERED`, {
    method: 'PUT',
    body: JSON.stringify({ range: `'Daily Action Sheet'!A${rowNumber}:X${rowNumber}`, majorDimension: 'ROWS', values: [values] })
  });
}

export async function archiveActionRow(values) {
  const encoded = encodeURIComponent(ARCHIVE_RANGE);
  return googleFetch(`/values/${encoded}:append?valueInputOption=USER_ENTERED&insertDataOption=INSERT_ROWS`, {
    method: 'POST',
